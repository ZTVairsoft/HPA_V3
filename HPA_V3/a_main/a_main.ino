...
#define TRASHOP 4  //управление ШИМ алгоритм для трассерки
#define BUBEN 5  //управление ШИМ алгоритм для бубна с мотором
...
#define LIGHTSLEEPTIME 20000 //20 секунд до ухода в легкий сон
...
bool isLSTr = false;                  //флаг для трассерки
bool isLD2 = false;                   //флаг для второй нагрузки типа бубен
...
*/
  ...
  int PCB = 2;  //Платапо умолчанию АК
  int HPAsystemType = 0;  //Тип ВВД системы по умолчанию
  ...
  bool isSoundModule = false;
  int SMVolume = 20;
  int SMReleaseMagSound = 0;
  int SMConnectMagSound = 0;
  int SMSingleShotSound = 0;
  int SMMachineGunSound = 0;
  float zap7;
  float zap8;
  float zap9;
...
EEManager mem1(WF);  // передаём нашу переменную (фактически её адрес)
EEManager mem2(WR);
EEManager mem3(Settings);